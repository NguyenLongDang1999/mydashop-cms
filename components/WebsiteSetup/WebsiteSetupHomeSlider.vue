...
// ** useHooks
const { data } = await useWebsiteSetupDetail(WEBSITE_SETUP.HOME_SLIDER)
const { isPending, mutateAsync } = useWebsiteSetupFormInput()

const { handleSubmit } = useForm({
    validationSchema: schema,
    initialValues: {
        home_slider: typeof data.value.value === 'string' ? JSON.parse(data.value.value) : []
    }
})
...
const onSubmit = handleSubmit(values => mutateAsync({
    id: data.value.id,
    slug: WEBSITE_SETUP.HOME_SLIDER,
