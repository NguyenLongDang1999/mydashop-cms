...
// ** useHooks
const { data } = await useWebsiteSetupDetail(WEBSITE_SETUP.HOME_BANNER1)
const { isPending, mutateAsync } = useWebsiteSetupFormInput()

const { handleSubmit } = useForm({
    validationSchema: schema,
    initialValues: {
        home_banner1: typeof data.value.value === 'string' ? JSON.parse(data.value.value) : []
    }
})
...
const onSubmit = handleSubmit(values => mutateAsync({
    id: data.value.id,
    slug: WEBSITE_SETUP.HOME_BANNER1,
    value: JSON.stringify(values.home_banner1)
}))
</script>
                ...
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12">
                        <div class="flex flex-col gap-4">
                            ...
                            >
                                <div class="col-span-12">
                                    <FormUpload :name="`home_banner1.${index}.image_uri`" />
                                </div>

                                <div class="col-span-6">
                                    <FormInput
                                        :label="label.image_link"
                                        :name="`home_banner1.${index}.image_link`"
                                    />
                                </div>
